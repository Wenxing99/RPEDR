...
super.data <- read.csv("Real_Data/Superconductivty.csv")
...
train.x <- as.matrix(subset(super.data[s,], select = -`critical_temp`))
train.y <- super.data[s,]$critical_temp

test.x <- as.matrix(subset(super.data[-s,], select = -`critical_temp`))
test.y <- super.data[-s,]$critical_temp
