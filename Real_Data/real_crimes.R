...
communities.data <- read.csv("Real_Data/communities.data", header = FALSE)
communities.data[communities.data == "?"] <- NA
communities.data <- communities.data[, colSums(is.na(communities.data)) == 0]
communities.data <- subset(communities.data, select = -c(`V1`, `V4`, `V5`))

## Setting d0 ##
d0 <- 3

set.seed(1000)
n <- nrow(communities.data)
s <- sample(n, 2*n/3)
p <- ncol(communities.data) - 1

train.x <- as.matrix(subset(communities.data[s,], select=-`V128`))
train.y <- communities.data[s,]$V128

test.x <- as.matrix(subset(communities.data[-s,], select=-`V128`))
test.y <- communities.data[-s,]$V128
...
sir.est.reg = predict(fit.sir, test.x %*% m.sir.dir)
...
phd.est.reg = predict(fit.phd, test.x %*% m.phd.dir)
...
### For reproducing results for DR, see https://github.com/JSongLab/SDR_HC ###
